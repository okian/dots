...
| `mail <to> [body…]` | send via Apple Mail; `<to>` is an email or a Contacts name; body from pipe and/or args; `-s` subject, `--cc/--bcc/--from`, `-a [file…]` to attach, `--draft` to review first |
| `fmail <to> [intent…]` | a model writes subject + body: words = intent, piped data = material it summarizes. Routes by size: short → on-device `fm`; long → Apple cloud (PCC), else Claude. `<to>` may be a Contacts name; `-a [file…]` attaches. Opens a Mail draft; `--send` ships it |
