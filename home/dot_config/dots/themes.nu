# themes.nu — theme registry + the `dots theme` switcher. Sourced by config.nu.
#
# One command retints everything. The active theme is captured in generated
# files under ~/.config/dots/ (NOT chezmoi-managed), which every tool reads:
#   theme           active theme slug
#   palette.json    full active theme entry (read by nushell + WezTerm)
#   active-theme.sh POSIX exports (BAT_THEME, STARSHIP_CONFIG) for zsh
#   starship.toml   committed starship config with the active palette selected
#   lazygit.yml     lazygit theme overlay (merged via LG_CONFIG_FILE)
# plus ~/.config/television/themes/dots.toml.
# Neovim and Doom read the slug and map it to their own real colorscheme.
# Everything falls back to the default below, so a fresh machine renders
# correctly before `dots theme` is ever run.

const THEME_DEFAULT = "catppuccin-mocha"

# --- Registry: slug -> { display name, per-tool ids, 14-colour palette } -----
# A parse-time constant (pure data) so it isn't rebuilt on every shell startup.
const THEME_REGISTRY = {
    catppuccin-mocha: {
      name: "Catppuccin Mocha"
      nvim: "catppuccin-mocha"  doom: "catppuccin"        doom_flavor: "mocha"
      bat: "Catppuccin Mocha"   starship: "catppuccin_mocha"
      palette: {
        bg: "#1e1e2e"  bg_dark: "#11111b"  surface: "#313244"  overlay: "#6c7086"
        fg: "#cdd6f4"  subtle: "#a6adc8"
        red: "#f38ba8"  green: "#a6e3a1"  yellow: "#f9e2af"  blue: "#89b4fa"
        magenta: "#cba6f7"  cyan: "#94e2d5"  orange: "#fab387"  accent: "#cba6f7"
      }
    }
    nord: {
      name: "Nord"
      nvim: "nord"  doom: "doom-nord"  doom_flavor: ""
      bat: "Nord"   starship: "nord"
      palette: {
        bg: "#2e3440"  bg_dark: "#272c36"  surface: "#3b4252"  overlay: "#4c566a"
        fg: "#d8dee9"  subtle: "#7b88a1"
        red: "#bf616a"  green: "#a3be8c"  yellow: "#ebcb8b"  blue: "#81a1c1"
        magenta: "#b48ead"  cyan: "#88c0d0"  orange: "#d08770"  accent: "#88c0d0"
      }
    }
    tokyo-night: {
      name: "Tokyo Night"
      nvim: "tokyonight-night"  doom: "doom-tokyo-night"  doom_flavor: ""
      bat: "base16"             starship: "tokyo_night"
      palette: {
        bg: "#1a1b26"  bg_dark: "#16161e"  surface: "#292e42"  overlay: "#565f89"
        fg: "#c0caf5"  subtle: "#a9b1d6"
        red: "#f7768e"  green: "#9ece6a"  yellow: "#e0af68"  blue: "#7aa2f7"
        magenta: "#bb9af7"  cyan: "#7dcfff"  orange: "#ff9e64"  accent: "#7aa2f7"
      }
    }
    gruvbox-dark: {
      name: "Gruvbox Dark"
      nvim: "gruvbox"  doom: "doom-gruvbox"  doom_flavor: ""
      bat: "gruvbox-dark"  starship: "gruvbox_dark"
      palette: {
        bg: "#282828"  bg_dark: "#1d2021"  surface: "#3c3836"  overlay: "#928374"
        fg: "#ebdbb2"  subtle: "#a89984"
        red: "#fb4934"  green: "#b8bb26"  yellow: "#fabd2f"  blue: "#83a598"
        magenta: "#d3869b"  cyan: "#8ec07c"  orange: "#fe8019"  accent: "#fabd2f"
      }
    }
}

def _theme_dir [] { $nu.home-dir | path join '.config' 'dots' }

# Active slug (validated against the registry; falls back to the default).
def _theme_current [] {
  let f = (_theme_dir | path join 'theme')
  if ($f | path exists) {
    let v = (open $f | str trim)
    if ($v in ($THEME_REGISTRY | columns)) { $v } else { $THEME_DEFAULT }
  } else { $THEME_DEFAULT }
}

# Active theme entry — from the generated palette.json if present, else the
# registry. Validated and merged over the registry entry, so a stale or corrupt
# generated file (older format, interrupted write) can never break config.nu —
# missing keys just fall back to the registry defaults.
def _theme_active [] {
  let fallback = ($THEME_REGISTRY | get (_theme_current))
  let f = (_theme_dir | path join 'palette.json')
  if not ($f | path exists) { return $fallback }
  let v = (try { open $f } catch { null })
  if not (($v | describe) | str starts-with 'record') { return $fallback }
  let vpal = (if ('palette' in $v) and (($v.palette | describe) | str starts-with 'record') { $v.palette } else { {} })
  $fallback | merge $v | merge { palette: ($fallback.palette | merge $vpal) }
}

# Build a nushell color_config record from a palette (used by config.nu).
def color_config_from_palette [p: record] {
  {
    separator: $p.overlay
    header: { fg: $p.subtle attr: b }
    row_index: $p.overlay
    empty: $p.overlay
    leading_trailing_space_bg: { attr: n }
    bool: $p.cyan
    int: $p.fg
    filesize: $p.orange
    duration: $p.orange
    date: $p.subtle
    range: $p.fg
    float: $p.fg
    string: $p.fg
    nothing: $p.overlay
    cell-path: $p.subtle
    hints: $p.overlay
    search_result: { fg: $p.bg bg: $p.yellow }
    shape_directory: { fg: $p.blue attr: b }
    shape_external: $p.cyan
    shape_internalcall: { fg: $p.cyan attr: b }
    shape_flag: { fg: $p.magenta attr: b }
    shape_string: $p.green
    shape_filepath: $p.cyan
    shape_globpattern: $p.cyan
    shape_int: $p.orange
    shape_literal: $p.blue
    shape_operator: $p.cyan
    shape_pipe: { fg: $p.magenta attr: b }
    shape_garbage: { fg: $p.bg bg: $p.red }
    shape_variable: $p.magenta
  }
}

# Television theme file (flat `key = '#hex'` TOML) for a palette. Written to
# ~/.config/television/themes/dots.toml; config.toml selects `theme = "dots"`.
def _television_theme [p: record] {
  ([
    "# generated by `dots theme` — selected via ~/.config/television/config.toml. DO NOT EDIT."
    $"background = '($p.bg)'"
    $"border_fg = '($p.surface)'"
    $"text_fg = '($p.fg)'"
    $"dimmed_text_fg = '($p.subtle)'"
    $"input_text_fg = '($p.fg)'"
    $"result_count_fg = '($p.magenta)'"
    $"result_name_fg = '($p.blue)'"
    $"result_line_number_fg = '($p.yellow)'"
    $"result_value_fg = '($p.subtle)'"
    $"selection_fg = '($p.fg)'"
    $"selection_bg = '($p.surface)'"
    $"match_fg = '($p.red)'"
    $"preview_title_fg = '($p.green)'"
    $"channel_mode_fg = '($p.bg_dark)'"
    $"channel_mode_bg = '($p.accent)'"
    $"remote_control_mode_fg = '($p.bg_dark)'"
    $"remote_control_mode_bg = '($p.green)'"
    $"action_picker_mode_fg = '($p.bg_dark)'"
    $"action_picker_mode_bg = '($p.blue)'"
  ] | str join (char nl)) + (char nl)
}

# lazygit theme overlay (~/.config/dots/lazygit.yml). lazygit merges this on top
# of the committed ~/.config/lazygit/config.yml via LG_CONFIG_FILE (env.nu /
# .zshrc), so only the colour theme is generated here — behaviour stays in the
# committed file. Layered last → wins, so lazygit retints with everything else.
def _lazygit_theme [p: record] {
  ([
    "# generated by `dots theme` — merged via LG_CONFIG_FILE. DO NOT EDIT."
    "gui:"
    "  theme:"
    $"    activeBorderColor: ['($p.accent)', bold]"
    $"    inactiveBorderColor: ['($p.overlay)']"
    $"    searchingActiveBorderColor: ['($p.yellow)', bold]"
    $"    optionsTextColor: ['($p.blue)']"
    $"    selectedLineBgColor: ['($p.surface)']"
    $"    cherryPickedCommitBgColor: ['($p.surface)']"
    $"    cherryPickedCommitFgColor: ['($p.accent)']"
    $"    markedBaseCommitBgColor: ['($p.surface)']"
    $"    markedBaseCommitFgColor: ['($p.yellow)']"
    $"    unstagedChangesColor: ['($p.red)']"
    $"    defaultFgColor: ['($p.fg)']"
  ] | str join (char nl)) + (char nl)
}

# POSIX exports for the zsh fallback shell.
def _theme_sh [t: record] {
  let ss = ($nu.home-dir | path join '.config' 'dots' 'starship.toml')
  ([
    "# generated by `dots theme` — sourced by ~/.zshrc. DO NOT EDIT."
    $"export BAT_THEME='($t.bat)'"
    $"export STARSHIP_CONFIG='($ss)'"
  ] | str join (char nl)) + (char nl)
}

# Regenerate every active-theme artifact for <slug>, then live-reload.
def _theme_apply [slug: string] {
  let t = ($THEME_REGISTRY | get $slug)
  let d = (_theme_dir)
  mkdir $d
  $slug | save -f ($d | path join 'theme')
  $t | to json | save -f ($d | path join 'palette.json')
  (_theme_sh $t) | save -f ($d | path join 'active-theme.sh')

  # television: write the "dots" theme its config.toml points at.
  let tv_themes = ($nu.home-dir | path join '.config' 'television' 'themes')
  mkdir $tv_themes
  (_television_theme $t.palette) | save -f ($tv_themes | path join 'dots.toml')

  # lazygit: write the theme overlay LG_CONFIG_FILE merges on top of config.yml.
  (_lazygit_theme $t.palette) | save -f ($d | path join 'lazygit.yml')

  # starship: write a copy with the active palette selected AND its colour block
  # GENERATED from the registry — so the registry is the single source of truth
  # and the committed starship.toml only carries the catppuccin fallback block.
  # Drop any committed `[palettes.*]` (everything from the marker on) and append
  # the one active block. The committed file stays untouched (no perpetual diff).
  let ss_src = ($nu.home-dir | path join '.config' 'starship.toml')
  if ($ss_src | path exists) {
    let body = (open --raw $ss_src | lines
      | take while {|l| not ($l | str starts-with '# --- Palettes') }
      | each {|l| if ($l | str trim | str starts-with 'palette ') { $'palette = "($t.starship)"' } else { $l } })
    let p = $t.palette
    let block = [
      "# --- Active palette (generated by `dots theme` from themes.nu) ----------"
      $"[palettes.($t.starship)]"
      $"red = \"($p.red)\""    $"green = \"($p.green)\""  $"yellow = \"($p.yellow)\""
      $"blue = \"($p.blue)\""  $"magenta = \"($p.magenta)\""  $"cyan = \"($p.cyan)\""
    ]
    (($body | append "") ++ $block | str join (char nl)) + (char nl) | save -f ($d | path join 'starship.toml')
  }

  _theme_reload $t
}

# Live-reload running apps where we can (best-effort, never fails).
def _theme_reload [t: record] {
  if (which emacsclient | is-not-empty) {
    let elisp = (if ($t.doom_flavor | is-empty) {
      "(load-theme '" + $t.doom + " t)"
    } else {
      "(progn (setq catppuccin-flavor '" + $t.doom_flavor + ") (load-theme '" + $t.doom + " t) (when (fboundp 'catppuccin-reload) (catppuccin-reload)))"
    })
    # silent + non-fatal when no Emacs daemon is running
    ^emacsclient --eval $elisp | complete | ignore
  }
  # WezTerm watches palette.json and reloads itself; starship rereads
  # STARSHIP_CONFIG on the next prompt.
}

# --- User-facing commands ---------------------------------------------------

# List available themes (● marks the active one).
def "dots theme list" [] {
  let cur = (_theme_current)
  $THEME_REGISTRY | transpose slug meta | each {|r|
    { "  ": (if $r.slug == $cur { "●" } else { " " }) theme: $r.slug name: $r.meta.name }
  }
}

# Switch the theme everywhere. e.g. dots theme set nord
def "dots theme set" [slug: string] {
  if ($slug not-in ($THEME_REGISTRY | columns)) {
    print $"unknown theme '($slug)'. Available: ($THEME_REGISTRY | columns | str join ', ')"
    return
  }
  _theme_apply $slug
  print $"==> theme → ($slug). WezTerm & a running Emacs retint live; new nu & nvim shells pick it up — run `exec nu` to retint this one."
}

# Bare `dots theme`: fuzzy-pick a theme (or just list if television is absent).
def "dots theme" [] {
  if (which tv | is-empty) { dots theme list; return }
  let pick = ($THEME_REGISTRY | columns | str join (char nl)
    | ^tv | str trim)
  if ($pick | is-not-empty) { dots theme set $pick }
}
